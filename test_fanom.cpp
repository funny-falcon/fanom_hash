...
static uint64_t seed[2] = {1, 2};
static int custom_seed = 0;

static int use_32 = 0;
static inline uint64_t
_hashsum_(const char* str, size_t len) {
	if (use_32 == 0) {
		return fanom64_string_hash2(str, len, seed[0], seed[1]);
	} else {
		return fanom32_string_hash2(str, len, (uint32_t)seed[0], (uint32_t)seed[1]);
	}
}
...
int main(int argc, char** argv) {
	...
	for (i=1; i<argc; i++) {
		if (strcmp(argv[i], "--help") == 0) {
			...
			if (custom_seed == sizeof(seed)/sizeof(seed[0]))
				goto usage;
			s = strtoll(argv[i], NULL, 0); /* strtol recognize hex */
			seed[custom_seed++] = s;
	...
	if (binary) {
		...
		while ((lsize = fread(lbuf, 1, binary, stdin)) > 0) {
			if (only_hash) {
				uint64_t hash;
				if (use_32 == 0) {
					hash = fanom64_string_hash2(lbuf, lsize, seed[0], seed[1]);
				} else {
					hash = fanom32_string_hash2(lbuf, lsize, seed[0], seed[1]);
				}
				if (only_hash == 2) {
					printf("%0*" PRIx64 "\n", use_32?8:16, hash);
				}
	...
	return 0;
usage:
	printf("Usage: %s [-seed N] [-x] [-b N] [-32] [-speed|-table] [-s] [-c] < input_file\n%s", argv[0],
	...
	"\t-table  - test hash table (all options below for this mode)\n" \
